...
$$.main_anon = {"":"Closure;box_0",
 call$1: function(position) {
  this.box_0.startPosition_0 = position;
...
$$.main_anon1 = {"":"Closure;box_0",
 call$1: function(position) {
  $.displayWatchPositionData(this.box_0.startPosition_0, position);
}
};
...
$._calculateDistance = function(lat1, long1, lat2, long2) {
  var LatDiff, LongDiff, a;
  LatDiff = $.div($.mul($.sub(lat2, lat1), 3.141592653589793), 180);
  LongDiff = $.div($.mul($.sub(long2, long1), 3.141592653589793), 180);
  a = $.pow($.sin($.div(LatDiff, 2)), 2) + $.cos($.div($.mul(lat1, 3.141592653589793), 180)) * $.cos($.div($.mul(lat2, 3.141592653589793), 180)) * $.pow($.sin($.div(LongDiff, 2)), 2);
  return 6371 * (2 * $.atan2($.sqrt(a), $.sqrt(1 - a))) * 0.6;
};

$._getTime = function(timestamp) {
  var date, t1;
  date = $.Date_Date$fromMillisecondsSinceEpoch(timestamp, false);
  t1 = $.getInterceptor(date).toString$0(date);
  t1 = $.getInterceptor$JSString(t1).split$1(t1, " ");
  t1 = $.getInterceptor$JSArray(t1).get$last(t1);
  return $.index($.getInterceptor$JSString(t1).split$1(t1, "."), 0);
};

$._displayData = function(subject, position) {
  var t1 = "<td><b>" + subject + "</b></td>\n  <td>" + $.S(position.get$coords().get$latitude()) + "&deg;</td>\n  <td>" + $.S(position.get$coords().get$longitude()) + "&deg;</td>\n  <td>" + $.S($._getTime(position.get$timestamp())) + "</td>\n  <td class=";
  return t1 + ($.gtB(position.get$coords().get$accuracy(), 100) ? "red" : "green") + ">" + $.S(position.get$coords().get$accuracy()) + " feet</td>";
};

$.displayCurrentPositionData = function(position) {
  var elem, t1;
  elem = $.TableRowElement_TableRowElement();
  elem.set$innerHtml($._displayData("Starting", position));
  t1 = $.query("#geo-data").get$children();
  $.getInterceptor$JSArray(t1).add$1(t1, elem);
};

$.displayWatchPositionData = function(startPosition, currentPosition) {
  var elem, t1, t2, t3, t4;
  elem = $.TableRowElement_TableRowElement();
  elem.set$innerHtml($._displayData("Current", currentPosition));
  t1 = $.query("#geo-data").get$children();
  $.getInterceptor$JSArray(t1).add$1(t1, elem);
  t1 = startPosition.get$coords();
  t2 = t1.get$latitude();
  t1 = t1.get$longitude();
  t3 = currentPosition.get$coords();
  t4 = $.S($.CONSTANT5.toStringAsFixed$1($._calculateDistance(t2, t1, t3.get$latitude(), t3.get$longitude()), 4)) + " miles";
  $.query("#distance").set$text(t4);
};
...
$.handleError = function(error) {
  ...
  switch (error.get$code()) {
    case 1:
      $.query("#error").set$text("The user did not grant permission to access location data.");
      break;
    case 2:
      $.query("#error").set$text("The browser could not determine your location");
      break;
    case 3:
      $.query("#error").set$text("The browser timed out before fetching your location");
      break;
    default:
      t1 = "There was an error in retreiving your location: " + $.S(error.get$message());
      $.query("#error").set$text(t1);
      break;
  }
};

$.main = function() {
  var t1, optional_params;
  t1 = {};
  t1.startPosition_0 = null;
  optional_params = $.makeLiteralMap(["timeout", 10000, "enableHighAccuracy", true]);
  $.window().get$navigator().get$geolocation().getCurrentPosition$3(new $.main_anon(t1), new $.main_anon0(), optional_params);
  $.window().get$navigator().get$geolocation().watchPosition$3(new $.main_anon1(t1), new $.main_anon2(), optional_params);
};
...
$.PositionError_PERMISSION_DENIED = 1;
$.PositionError_POSITION_UNAVAILABLE = 2;
$.PositionError_TIMEOUT = 3;
...
$.$defineNativeClass('HTMLAnchorElement', {"":"coords>",
 toString$0: function() {
  return this.toString();
}
});

$.$defineNativeClass('HTMLAppletElement', {"":"code>"
});

$.$defineNativeClass('HTMLAreaElement', {"":"coords>"
});
...
$.$defineNativeClass('Coordinates', {"":"accuracy>,latitude>,longitude>"
});
...
$.$defineNativeClass('Geolocation', {
 ...
 watchPosition$3: function(successCallback, errorCallback, options) {
  return this.watchPosition($.convertDartClosureToJS(successCallback, 1),$.convertDartClosureToJS(errorCallback, 1),options);
}
});

$.$defineNativeClass('Geoposition', {"":"coords>,timestamp>"
});
...
$.$defineNativeClass('HTMLObjectElement', {"":"code>"
});
...
//@ sourceMappingURL=distance_tracker.dart.js.map
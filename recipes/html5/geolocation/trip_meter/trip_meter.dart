...
void calcCurrentPosition(Geoposition position) {
  query("#start").innerHtml = _displayData("Starting", position);
}

void calcWatchPosition(Geoposition startPosition, Geoposition currentPosition) {
  query("#current").innerHtml = _displayData("Current", currentPosition);
...
void main(){
  ...
  window.navigator.geolocation.getCurrentPosition(
    (Geoposition position) {
      ...
      calcCurrentPosition(position);
    },
    (error) => handleError(error)
  );
 
  window.navigator.geolocation.watchPosition((Geoposition position) {
    calcWatchPosition(startPosition, position);
    },
    (error) => handleError(error)
  );
}
